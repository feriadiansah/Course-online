...
@section('title', 'All Course - Obito Online Learning Platform')
@section('content')
    <x-navigation-auth />
            ...
            <div id="tabs-content-container" class="mt-1">
                ...
                @foreach ($courseByCategory as $category => $courses)
                    <div id="{{ Str::slug($category) }}-content"
                        class="tab-btn group {{ $loop->first ? '' : 'hidden' }} tab-content grid grid-cols-4 gap-5">
                        @forelse($courses as $course)
                            <x-course-card :course="$course"/>
                        @empty
                            <p>belum ada kelas pada category ini</p>
                        @endforelse
                    </div>
                @endforeach
            </div>
